...
/*if (!defined('myCRED_VERSION')) {
    $likebtn_active_plugins = get_option('active_plugins');
    foreach ($likebtn_active_plugins as $active_plugin) {
        if (strstr($active_plugin, 'mycred/mycred.php')) {
            if (file_exists(dirname(__FILE__) . '/../mycred/mycred.php')) {
                require_once(dirname(__FILE__) . '/../mycred/mycred.php');
            }
            break;
        }
    }
...
function likebtn_admin_help() 
{
    ...
    ?>
    <div>
        <iframe width="100%" height="315" src="https://www.youtube.com/embed/JpMYoKPPbyM" frameborder="0" allowfullscreen></iframe>
        <ul>
...
function likebtn_comment_text($content) {
    ...
    $html = _likebtn_get_markup($real_entity_name, $comment_id, $values, $entity_name);
...
function _likebtn_save_vote($identifier, $type, $old_type = 0, $act = 'u')
{
    global $wpdb;
            ...
            if (isset($body['data']['type'])) {
                ...
                if (!empty($settings['s'])) {
                    ...
                    if (!empty($s['i'])) {
                        ...
                        _likebtn_save_vote($s['i'], $vote_type, $vote_old_type, $vote_act);
                        //}
                    }
                }
            }
